/*
 * Copyright (C) 13.03.2006 | Naoghuman (Peter Rogge) | peter.rogge@yahoo.de
 *
...
/**
 * The <code>GridLayoutLight</code> class is a layout manager that 
 ...
 * <p>
 * @author Naoghuman (Peter Rogge) | Copyright (c) | 13.03.2006
 * 
 * @change 08.10.2006
 * @version 1.0
 */
public final class GridLayoutLight implements LayoutManager, java.io.Serializable {
	 ...
	 * short, where '0' means 'any number' meaning that the number of Columns
	 ...
	 */
	public GridLayoutLight() { this(1, 0, 0, 0); }
	 ...
	 *            columns.
	 */
	public GridLayoutLight(final int rows, final int cols) { this(rows, cols, 0, 0); }
	 ...
	 * @exception IllegalArgumentException
	 *                if the value of both <code>rows</code> and
	 *                <code>cols</code> is set to zero or one of <code>top</code>,
	 ...
	 * order to satisfy the constraints of the <code>GridLayoutLight</code> object.
	 * <p>
	...
	public final void layoutContainer(final Container parent) {
		...
		synchronized (parent.getTreeLock()) {
			...
			if (ltr) {
				for (
						int c = 0, x = insets.left + left;
						c < ncols;
						c++, x += w + hgap
				) {
					for (
							int r = 0, y = insets.top + top;
							r < nrows;
							r++, y += h + vgap
					) {
				...
				for (
						int c = 0, x = parent.getWidth() - insets.right - w - right;
						c < ncols;
						c++, x -= w + hgap
				) {
					for (
