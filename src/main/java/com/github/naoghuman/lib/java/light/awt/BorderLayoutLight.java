...
/**
 * A <code>BorderLayoutLight</code> lays out a container, arranging and resizing
 ...
 * @version 1.0
 */
public final class BorderLayoutLight extends Object implements LayoutManager2,
	...
	/**
	 * Constructs a <code>BorderLayoutLight</code> with the horizontal gaps
	 * between components. The horizontal gap is specified by <code>hgap</code>.
	 */
	private short hgap = ZERO;

	/**
	 * Constructs a <code>BorderLayoutLight</code> with the vertical gaps between
	 * components. The vertical gap is specified by <code>vgap</code>.
	 */
	private short vgap = ZERO;

	/**
	 * Constructs a <code>BorderLayoutLight</code> with a border at the bottom
	 * from all components. The bottom border is specified by <code>bottom</code>.
	 */
	private short bottom = ZERO;

	/**
	 * Constructs a <code>BorderLayoutLight</code> with a border at the left
	 * from all components. The left border is specified by <code>left</code>.
	 */
	private short left = ZERO;

	/**
	 * Constructs a <code>BorderLayoutLight</code> with a border at the right
	 * from all components. The right border is specified by <code>right</code>.
	 */
	private short right = ZERO;

	/**
	 * Constructs a <code>BorderLayoutLight</code> with a border at the top
	 * from all components. The top border is specified by <code>top</code>.
	 */
	private short top = ZERO;

	/**
	 * Constant to specify components location to be the north portion of the
	 * <code>BorderLayoutLight</code>.
	 * 
	 * @serial
	...
	/**
	 * Constructs a new <code>PRoBorderLayout</code> with no gaps between
	 * and no border around the components.
	 */
	public BorderLayoutLight() { this(ZERO, ZERO); }
	 ...
	 */
	public BorderLayoutLight(final int hgap, final int vgap) {
	...
	/**
	 * Constructs a new <code>PRoBorderLayout</code> with no gaps between
	 * and a border around the components.<p>
	 ...
	 * The horizontal gap is specified by <code>hgap</code> and the vertical
	 * gap is specified by <code>vgap</code>.<br>
	 * The border around the components  is specified by <code>top</code>,
	 * <code>left</code>, <code>bottom</code> and <code>right</code>.
	 * 
	 * @param hgap the horizontal gap.
	 ...
	 * constraint object. For <code>BorderLayoutLight</code>, the constraint
	 * must be one of the following constants: NORTH, SOUTH, EAST, WEST or CENTER.<p>
	...
	private final Component getChild(final String key, final boolean ltr) {
		...
		if (key.equals(NORTH)) { comp = north; }
		else if (key.equals(SOUTH) ) { comp = south; }
		else if (key.equals(WEST)  ) { comp = west; }
		else if (key.equals(EAST)  ) { comp = east; }
		else if (key.equals(CENTER)) { comp = center; }

		if (comp != null && !comp.isVisible()) { comp = null; }
	...
	public final void addLayoutComponent(String name, final Component comp) {
		...
		synchronized (comp.getTreeLock()) {
			...
			if (contraint == null) { contraint = CENTER; }

			/*
			 * Assign the component to one of the known regions of the layout.
			 */
			if (contraint.equals(CENTER)    ) { center = comp; }
			else if (contraint.equals(NORTH)) { north = comp; }
			else if (contraint.equals(SOUTH)) { south = comp; }
			else if (contraint.equals(EAST) ) { east = comp; }
			else if (contraint.equals(WEST) ) { west = comp; }
			else {
				throw new IllegalArgumentException(
	 ...
	 * object. The NORTH and SOUTH components, if any, are placed at the top
	 * and bottom of the container, respectively. The WEST and EAST components
	 * are then placed on the left and right, respectively. Finally, the CENTER
     ...
     */
    private final int max(final int a, final int b) { return (a >= b) ? a : b;  }

	/**
	 * Removes the specified component from this <code>BorderLayoutLight</code>.
	...
	public final void removeLayoutComponent(final Component comp) {
		...
		synchronized (comp.getTreeLock()) {
			...
			if (comp.equals(CENTER)) { center = null; }
			else if (comp.equals(NORTH)) { north = null; }
			else if (comp.equals(SOUTH)) { south = null; }
			else if (comp.equals(EAST) ) { east = null; }
			else if (comp.equals(WEST) ) { west = null; }
		}
	}
