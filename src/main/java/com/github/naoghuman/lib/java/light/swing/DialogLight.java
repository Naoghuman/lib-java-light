/*
 * Copyright (C) 01.11.2006 | Naoghuman (Peter Rogge) | peter.rogge@yahoo.de
 *
...
/**
 * Mit <code>DialogLight</code> können Sie Ihr eigenes Dialogfenter generieren.<p>
 * 
 * Als <code>Defaultlayout</code> ist ein <code>BorderLayoutLight</code> mit
 * 10px Abstand zwischen den Komponenten und einen 10px großen leeren Rahmen
 * um alle Komponenten implementiert.<p>
 * 
 * Über die fünf <code>initDirection-Methoden</code> (Norden, Süden, Osten,
 * Westen und Zentrum) können Komponenten den jeweiligen Bereichen
 * hinzugefügt werden. Die Methoden initCenter() und initSouth() sind
 * abstract, d.h. sie müssen implementiert werden, während die
 * restlichen initDirection-Methoden optional überlagert werden können.
 * Natürlich können Sie auch die Methode <code>init()</code> einfach
 * überlagern, damit wären die <code>Defaulteinstellungen</code> der
 * fünf <code>initDirection-Methoden</code> in Ihren Händen.<p>
 * 
 * Über die beiden Methoden <code>closePRoDialog()</code> und
 * <code>showPRoDialog()</code> können Sie sich den DialogLight anzeigen oder
 entfernen lassen.<p>
 * 
 * <b>Hinweis:</b><br>
 * Sollte der leere Rahmen um alle Komponenten oder der Zwischenabstand
 * verändert werden, so ist über die Methode
 * <code>setLayout(LayoutManager)</code> ein neues Layout mit den Maßen
 * zu implementieren.<p>
 * 
 * <b>Benötigte Klassen:</b><br>
 *  - <code>de.pro.java.awt.BorderLayoutLight</code><br>
 *  - <code>de.pro.java.swing.PanelLight</code><p>
 *           
 * @author Naoghuman (Peter Rogge) | Copyright (c) | 01.11.2006
 * @version 2.0
 * @change 21.02.2007
 * @change 16.05.2007
 */
public abstract class DialogLight extends Dialog implements WindowConstants {
	 ...
	 * <code>DialogLight</code> created with null owners.
	 */
	...
	/**
     * Returns true if newly created <code>DialogLight</code>s should have their
	...
	/**
     * Provides a hint as to whether or not newly created <code>DialogLight</code>s
     ...
     * windows, then newly created <code>DialogLight</code>s will have their
     ...
     * <p>
 You can get the same effect on a single DialogLight by doing the following:
 <pre>
    DialogLight dialog = new DialogLight();
    ...
    protected boolean init = Boolean.FALSE;
	...
	protected PanelLight ppPRoDialog = null;
	 ...
	 */
	public DialogLight() throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Dialog owner) throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Dialog owner, final boolean modal)
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Dialog owner, final String title)
	throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Dialog owner, final String title, final boolean modal)
			throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Frame owner) throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Frame owner, final boolean modal)
	throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Frame owner, final String title)
	throws HeadlessException {
	 ...
	 * @see JComponent#getDefaultLocale
	 */
	public DialogLight(final Frame owner, final String title, final boolean modal)
			throws HeadlessException {

		super((owner == null ? DialogLight.getSharedOwnerFrame(): owner),
				title, modal
				);
		
		if (owner == null) {
			
			super.addWindowListener((WindowListener)DialogLight.getSharedOwnerFrame()
					);
		}
	...
	/**
	 * Der <code>DialogLight</code> wird geschlossen.
	 */
	...
	/**
	 * Initialisiert den <code>DialogLight</code>.
	 * 
	 * Komponenten des <code>PRoDialogs</code> werden den verschiedenen
	 * <code>Ausrichtung</code> des Layouts <code>BorderLayoutLight</code>
	 * zugeordnet.<p>
	 * 
	 * Die Methode <code>initSouth()</code> muss implementiert werden, während die
	 * restlichen Ausrichtungen optional sind.<p>
	 * 
	 * <b>Hinweis:</b><br>
	 * Wenn über die Methode <code>setLayout(LayoutManager)</code> ein
	 * anderes Layout implementiert wird, dann ist diese Methode zu überlagern,
	 * damit die neuen Komponenten entsprechend angeordnet werden.
	 */
	protected void init() {
		...
		if (jc != null) { ppPRoDialog.add(jc, BorderLayoutLight.NORTH); }
		
		jc = this.initEast();
		if (jc != null) { ppPRoDialog.add(jc, BorderLayoutLight.EAST); }
		
		jc = this.initWest();
		if (jc != null) { ppPRoDialog.add(jc, BorderLayoutLight.WEST); }
		
		jc = this.initCenter();
		if (jc != null) { ppPRoDialog.add(jc, BorderLayoutLight.CENTER); }
		
		jc = this.initSouth();
		if (jc != null) { ppPRoDialog.add(jc, BorderLayoutLight.SOUTH); }
		
		this.getContentPane().add(ppPRoDialog, BorderLayoutLight.CENTER);
	...
	/**
	 * Optionale Methode um Komponenten im <code>Zentrum</code> des
	 * <code>PRoDialogs</code> implementiert.<p>
	 */
	protected JComponent initCenter() { return null; }
	
	/**
	 * Optionale Methode um Komponenten im <code>Osten</code> des
	 * <code>PRoDialogs</code>  zu implementieren.
	 */
	protected JComponent initEast() { return null; }
	
	/**
	 * Optionale Methode um Komponenten im <code>Norden</code> des
	 * <code>PRoDialogs</code> zu implementieren.
	 */
	protected JComponent initNorth() { return null; }
	
	/**
	 * Called by the constructors to init the <code>DialogLight</code> properly.
	 */
	private final void initPRoDialog() {

		super.enableEvents(AWTEvent.KEY_EVENT_MASK | AWTEvent.WINDOW_EVENT_MASK);
		super.setLayout(new BorderLayoutLight());
		...
		super.addWindowListener(new WindowAdapter() {
			public final void windowClosing(final WindowEvent event) {
				DialogLight.this.closePRoDialog();
			}
		});
		
		this.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
		
		ppPRoDialog = new PanelLight(
				new BorderLayoutLight(5, 5, 5, 5, 5, 5), Boolean.TRUE);
	...
	/**
	 * Optionale Methode um Komponenten im <code>Süden</code> des
	 * <code>PRoDialogs</code> implementiert.<p>
	 */
	protected JComponent initSouth() { return null; }

	/**
	 * Optionale Methode um Komponenten im <code>Westen</code> des
	 * <code>PRoDialogs</code> zu implementieren.
	 */
	protected JComponent initWest() { return null; }

	/**
	 * Returns a string representation of this <code>DialogLight</code>. This
	 * method is intended to be used only for debugging purposes, and the
	 * content and format of the returned string may vary between
	 * implementations. The returned string may be empty but may not be
	 * <code>null</code>.
	 * 
	 * @return a string representation of this <code>DialogLight</code>.
	 */
	@Override
	protected String paramString() {
		...
		final String rootPaneCheckingEnabledString = (
	 ...
	 * <code>contentPane</code>. This will do nothing if <code>comp</code>
	 * is not a child of the <code>DialogLight</code> or <code>contentPane</code>.
	 * 
	...
	/**
	 * Sets the <code>contentPane</code> property. This method is called by
	 * the constructor.
	 * <p>
	 ...
	 * Do not do anything - require
	 * the program to handle the operation in the <code>windowClosing</code>
	 ...
	 * forward the call to the <code>contentPane</code>. Refer to
	 * {@link javax.swing.RootPaneContainer} for more information.<p>
	 * 
	 * <b>Wichtig:</b><br>
	 * Der neue <code>LayoutManager</code> sollte ein
	 * <code>BorderLayoutLight</code> sein, da das Hinzufügen der Komponenten
	 * in der Methode <code>init()</code> sich auf das Layout
	 * <code>BorderLayoutLight</code> bezieht.<br>
	 * Möchten Sie ein anderes Layout implementieren, dann ist die
	 * Methode <code>init()</code> zu überlagern.
	 * 
	...
	protected void setRootPane(final JRootPane root) {
		...
		if (rootPane != null) { this.remove(rootPane); }
		rootPane = root;
	...
	/**
	 * Der <code>DialogLight</code> wird angezeigt.
	 */
	public void showPRoDialog() { super.setVisible(Boolean.TRUE); }
	 ...
	 */
	@Override
	public void update(final Graphics g) { super.paint(g); }
}
